...
class ProsthesisRandomizer(DomainRandomizer):
    ...
    def init_state(self,
        ...
        return ProsthesisRandomizerState(prosthesis_joint_stiffness=backend.array([0.0] * len(self._joint_indices)),
                                      prosthesis_dof_damping=backend.array([0.0] * len(self._dof_indices)),
                                      prosthesis_body_position=backend.array([0.0, 0.0, 0.0] * len(self._body_indices)),
                                      prosthesis_body_orientation=backend.array([1.0, 0.0, 0.0, 0.0] * len(self._body_indices)),
                                      )
    ...
    def update(self,
        ...
        body_indices = list(self._body_indices.values())
        ...
        if backend == jnp:
            ...
            body_pos = model.body_pos.at[jnp.array(body_indices)].set(model.body_pos[jnp.array(body_indices)] + domrand_state.prosthesis_body_position)
            body_quat = model.body_quat.at[jnp.array(body_indices)].set(model.body_quat[jnp.array(body_indices)] +domrand_state.prosthesis_body_orientation)
        else:
            ...
            body_pos[body_indices] = domrand_state.prosthesis_body_position
            body_quat = self._init_prosthesis_body_orientation.copy()
            body_quat[body_indices] = domrand_state.prosthesis_body_orientation
        ...
        # jax.debug.print("model.body_pos: {body_pos}", body_pos=model.body_pos)
    ...
    def _sample_joint_stiffness(self, model: Union[MjModel, Model],
        ...
        sampled_stiffness = (
            stiffness_min + (stiffness_max - stiffness_min) * interpolation
            if self.rand_conf["randomize_prosthesis_joint_stiffness"]
            else model.jnt_stiffness[self._joint_indices.values()]
        )
    ...
    def _sample_geom_position(self, model: Union[MjModel, Model],
        ...
        n_bodies = len(self._body_indices.values())
        ...
        if backend == jnp:
            ...
            interpolation = jax.random.uniform(_k, shape=(ncoordinates, n_bodies))
            carry = carry.replace(key=key)
        else:
            interpolation = np.random.uniform(size=(ncoordinates, n_bodies ))
        ...
        if self.rand_conf["randomize_prosthesis_body_position"]:
            if backend == np:
                ...
                indices = jnp.array(list(self._body_indices.values()))
                sampled_position = model.body_pos.at[indices].get()
                def update_position(pos, interp):
                    for i in range(ncoordinates):
                        if directions[i] == 'x':
                            pos = pos.at[..., 0].set(interp[..., i])
                        elif directions[i] == 'y':
                            pos = pos.at[..., 1].set(interp[..., i])
                        elif directions[i] == 'z':
                            pos = pos.at[..., 2].set(interp[..., i])
                    return pos

                sampled_position = jax.vmap(update_position)(sampled_position, sampled_position_interp)
        # else: 
        #     sampled_position = backend.array([0,0,0]* n_bodies)
    ...
    def _sample_joint_orientation(self, model: Union[MjModel, Model],
        ...
        if backend == np:
            init_orientation = self._init_prosthesis_body_orientation[list(self._body_indices.values())].copy() # Format: (w,x,y,z)
        ...
        if self.rand_conf["randomize_prosthesis_body_orientation"]:
            for i in range(len(directions)):
                if backend == np:
                    if directions[i] == 'x':
                        orientation_euler[:, 0] = sampled_orientation_euler[:, i]
                    elif directions[i] == 'y':
                        orientation_euler[:, 1] = sampled_orientation_euler[:, i]
                    elif directions[i] == 'z':
                        orientation_euler[:, 2] = sampled_orientation_euler[:, i]
                elif backend == jnp:
                    def update_orientation(euler, interp):
                        for i in range(ndirections):
                            if directions[i] == 'x':
                                euler = euler.at[..., 0].set(interp[..., i])
                            elif directions[i] == 'y':
                                euler = euler.at[..., 1].set(interp[..., i])
                            elif directions[i] == 'z':
                                euler = euler.at[..., 2].set(interp[..., i])
                        return euler
        ...
        if self.rand_conf["randomize_prosthesis_body_orientation"]:
            ...
            sampled_orientation = self._init_prosthesis_body_orientation[list(self._body_indices.values())].copy()
            # sampled_orientation = backend.array([1.0,0.0,0.0,0.0] * n_bodies)  # Default orientation (w,x,y,z)
