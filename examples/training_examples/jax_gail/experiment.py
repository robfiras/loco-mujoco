...
def experiment(config: DictConfig):
    try:
        ...
        if not os.path.exists(expert_dataset_path):
            ...
            new_traj = env._traj.replace(obs_container=env.obs_container, transitions=expert_dataset)
        ...
        agent_state = jax.vmap(lambda r: GAILJax.init_agent_state(env, agent_conf, r))(_rng) \
            if config.experiment.n_seeds > 1 else GAILJax.init_agent_state(env, agent_conf, _rng)
