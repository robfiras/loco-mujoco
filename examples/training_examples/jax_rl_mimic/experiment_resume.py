...
from omegaconf import DictConfig, OmegaConf, open_dict  # open_dict needed for total_timesteps override
...
def experiment(config: DictConfig):
    try:
        ...
        # compute n_chunks from actual num_updates to avoid rounding mismatch
        updates_per_chunk = int(agent_conf.config.experiment.num_updates)
        total_updates = total_timesteps // int(config.experiment.num_steps) // int(config.experiment.num_envs)
            ...
            if not config.experiment.debug:
                ...
                for i in range(len(training_metrics.mean_episode_return)):
                    ...
                    if (i + 1) % config.experiment.validation_interval == 0 and config.experiment.validation.active:
                        ...
                        run.log({"Metric for Sweep": site_rpos + site_rrotvec + site_rvel}, step=step)
